...
class TenderController {
  static Tender_insert = async (req, res) => {
    try {
      ...
      res.status(200).json({ status: "SUCCESS", message: "Tender Registration Successfull", savetender })
    }
    catch (error) {
  ...
  static updateTender = async (req, res) => {
    try {
      ...
      if (!data) {
        ...
        res.status(404).json({ status: "Failed", message: "Data not provided" });
      } else {
        // Assuming you have a Tender model, you can update it like this
        const updatedTender = await TenderModel.findByIdAndUpdate(req.params.id, data, { new: true });

        if (!updatedTender) {
          // If the tender with the given ID is not found, send a 404 error
          res.status(404).json({ status: "Failed", message: "Tender not found" });
        } else {
  ...
  static deleteTender = async (req, res) => {
    try {
      ...
      const userExit = await TenderModel.findById(id)
      if (!userExit) {
